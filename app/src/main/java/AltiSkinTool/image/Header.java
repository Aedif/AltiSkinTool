...
public class Header {
	...
	private void readSpriteInfo() {
		...
		for (int i = 0; i < numberOfSprites; i++) {
			...
			try {
				spriteName = new String(b_name, "UTF-8");
	...
	public byte[] getBytes() {
		...
		for (Sprite sprite : sprites) {
			bb.clear();
			bb.putShort((short) sprite.spriteName.length());
			bb.position(0);
			bb.get(b_header, hIndex, 2);
			hIndex += 2;
			try {
			...
			b4.putInt(sprite.width);
			b4.position(0);
			b4.get(b_header, hIndex, 4);
			hIndex += 4;
			b4.clear();

			b4.putInt(sprite.height);
			b4.position(0);
			b4.get(b_header, hIndex, 4);
			hIndex += 4;
		}
		...
		bb.putInt(repeatType);
		bb.position(0);
		bb.get(b_header, hIndex, 4);
		hIndex += 4;
		bb.clear();

		bb.putInt(filterType);
		bb.position(0);
		bb.get(b_header, hIndex, 4);
		hIndex += 4;
		bb.clear();

		bb.putInt(numberOfTextures);
		bb.position(0);
		bb.get(b_header, hIndex, 4);
		hIndex += 4;
		bb.clear();

		bb.putInt(textureWidth);
		bb.position(0);
		bb.get(b_header, hIndex, 4);
		hIndex += 4;
		bb.clear();

		bb.putInt(textureHeight);
		bb.position(0);
		bb.get(b_header, hIndex, 4);
