 ...
 * optionally sort the images you want to add by size
 * (e.g. area) then insert each image via a call to
 * {@link #insertImage(String, BufferedImage)}. When you are done with inserting
 * images you can call {@link #getImage()} for the {@link BufferedImage} that
 * holds the packed images. Additionally you can get a
 * <code>Map<String, Rectangle></code> where the keys the names you specified
 * when inserting and the values are the rectangles
 * within the packed image where that specific image is located. All things are
 * given in pixels.
 * </p>
 * 
 * <p>
 * See the {@link #main(String[])} method for an example that will generate 100
 * random images, pack them and then output the
 ...
 * pixels of an inserted image so that there is no
 * bleeding of neighbouring pixels when using the packed image as a texture. You
 * can specify the padding as well as whether to
...
public class ImagePacker {
     ...
     * <code>width</code> by <code>height</code> image.
     * <code>padding</code> specifies the minimum number of pixels to insert between
     * images. <code>border</code> will duplicate the
     * border pixels of the inserted images to avoid seams when rendering with
    ...
    /**
     * <p>
     * Inserts the given image. You can later on retrieve the images position in the
     * output image via the supplied name and the
    ...
    public void insertImage(String name, BufferedImage image) {
        ...
        if (duplicateBorder) {
            ...
            g.drawImage(image, rect.x + rect.width, rect.y, rect.x + rect.width + 1, rect.y + rect.height,
                    image.getWidth() - 1, 0,
                    image.getWidth(), image.getHeight(), null);

            g.drawImage(image, rect.x - 1, rect.y - 1, rect.x, rect.y, 0, 0, 1, 1, null);
            g.drawImage(image, rect.x + rect.width, rect.y - 1, rect.x + rect.width + 1, rect.y, image.getWidth() - 1,
                    0,
                    image.getWidth(), 1, null);

            g.drawImage(image, rect.x - 1, rect.y + rect.height, rect.x, rect.y + rect.height + 1, 0,
                    image.getHeight() - 1, 1,
                    image.getHeight(), null);
            g.drawImage(image, rect.x + rect.width, rect.y + rect.height, rect.x + rect.width + 1,
