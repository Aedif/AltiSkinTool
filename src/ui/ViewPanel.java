    ...
    class DrawPanel extends JPanel {
            ...
            if(poly[j] != null && polyON){
                ...
                if(coord != null){
                    for(int i = 0; i<coord.length-3; i+=2){
                        ...
                        g.drawLine((int)(x_centre + coord[i]   - dimensions[0]/2.0f),(int)(y_centre + coord[i+1] - dimensions[1]/2.0f), 
                                   (int)(x_centre + coord[i+2] - dimensions[0]/2.0f),(int)(y_centre + coord[i+3] - dimensions[1]/2.0f));
                    }
                    g.drawLine((int)(x_centre + coord[coord.length-2]   - dimensions[0]/2.0f),(int)(y_centre + coord[coord.length-1] - dimensions[1]/2.0f), 
                           (int)(x_centre + coord[0] - dimensions[0]/2.0f),(int)(y_centre + coord[1] - dimensions[1]/2.0f));
                }
            }
        }
        ...
        private void drawTexture(Graphics g,  Poly[] poly, BufferedImage[] text, int j){
            ...
            if(poly[j].scale!= 1.0){
                ...
                if(outlineON){
                    g.drawRect((int)(x_centre+poly[j].translateX), (int)(y_centre+poly[j].translateY), transText.getWidth(), transText.getHeight()); // TEMP
                    g.setColor(Color.WHITE);
                    g.drawRect((int)(x_centre-(PolyWriter.customSpriteSize/2)), (int)(y_centre-(PolyWriter.customSpriteSize/2)),
